...
use crate::moduleref::{ModuleRef, LockedModuleRefCache, ModuleRefCache};
use crate::parser;
...
impl ModuleGraph {
    ...
    pub fn add<T: IntoIterator<Item=String>>(&self, filepath: &str, pkg: &str, module: &str,
                                             deps: T, is_ns_pkg_init: bool) {
        ...
        let module_ref = if nspkg && !is_local {
            ...
            self.modules_refs.get_or_create(ustr(""), ustr(module), None)
        } else {
    ...
    fn to_module_no_cache(&self, pkg_path: &str, mut dep: Ustr, ref_pkg: Option<Ustr>) -> Option<ModuleRef> {
        ...
        for _ in 0..2 {
            ...
            if let Some(r) = self.modules_refs.ref_for_fs(candidate_init) {
                ...
                return Some(self.modules_refs.get_or_create(candidate_init, dep, ref_pkg))
            } else if let Some((dir, name)) = candidate_module.rsplit_once('/') {
                if self.exists_case_sensitive(dir, name) {
                    return Some(self.modules_refs.get_or_create(candidate_module, dep, ref_pkg))
                }
            }
    ...
    pub fn add_dynamic_dependencies(&self, dynamic_edges: HashMap<String, HashSet<String>>) {
        for (m, deps) in dynamic_edges {
            if let Some(r) = self.modules_refs.ref_for_py(ustr(&m), None) {
                let rdeps = deps.iter()
                    .filter_map(|d| self.module_or_parent(&d));
